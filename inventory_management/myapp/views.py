...
def create_item(request):
    ...
    if serializer.is_valid():
        if Item.objects.filter(name=serializer.validated_data['name']).exists():
            logger.warning(f'Item creation failed: {serializer.validated_data["name"]} already exists.')
...
def read_item(request, item_id):
    ...
    try:
        ...
        logger.error(f'Item with ID {item_id} not found for reading.')
...
# # --------- Redis cache for below code currently i am not implemented the redis
